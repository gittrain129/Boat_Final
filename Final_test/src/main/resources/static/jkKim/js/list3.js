...
console.log('go2');
	...
	const data = `BOARD_EMPNO=${empno}&page=${page}&dept='부서별보기'&order='정렬옵션'`;
	console.log(data)
	...
	const data = `BOARD_EMPNO=${empno}&page=${page}&dept='부서별보기'&order='정렬옵션'&option1=${option1}&search1=${search1}`;
...
//=============================================================
  ...
  $('#drop-dept li a').click(function(event) {
  	var dept = $(this).text();
  	var order = $('#drop-order li.active a').text();
  	var state = $('#hidden-state').val();
  	var empno = $('#hidden-empno').val();
  	if(state == 'main'){ //기본화면에서 드롭다운 ajax
  	console.log(order)
  	console.log(dept)
$.ajax({
      url: '../board/list_ajax',
